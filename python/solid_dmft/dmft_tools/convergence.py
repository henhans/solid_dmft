...
def prep_conv_obs(h5_archive, ghostGA=False):
    """
...
def prep_conv_file(general_params, sum_k, ghostGA=False):
    ...
    headers = _generate_header(general_params, sum_k, ghostGA)
...
def check_convergence(n_inequiv_shells, general_params, conv_obs, ghostGA=False):
    ...
    if ghostGA:
        return None
    elif (general_params['occ_conv_crit'] <= 0.0 and general_params['gimp_conv_crit'] <= 0.0
